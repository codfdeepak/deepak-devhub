...
const isOwnerRequest = (req) => String(req?.user?.role || '').toLowerCase() === 'owner'
...
const login = async (req, res) => {
  try {
    ...
    if (approvalStatus !== 'approved') {
      return res.status(403).json({ message: 'Account pending approval from owner.' })
    }
...
const listUsers = async (req, res) => {
  try {
    ...
    const users = await User.find({})
      .select('fullName mobile email role isActive approvalStatus createdAt updatedAt')
      .sort({ createdAt: -1 })
...
const updateUserStatus = async (req, res) => {
  try {
    ...
    if (String(req.user.sub) === String(userId) && !isActive) {
      return res.status(400).json({ message: 'Owner cannot deactivate own account' })
    }

    const user = await User.findById(userId)
    if (!user) return res.status(404).json({ message: 'User not found' })
    if (String(user.role || '').toLowerCase() === 'owner') {
      return res.status(400).json({ message: 'Owner account status cannot be changed' })
    }
...
const updateUserApproval = async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ message: 'User not found' })
    if (String(user.role || '').toLowerCase() === 'owner') {
      return res.status(400).json({ message: 'Owner approval cannot be changed' })
    }
...
const deleteUser = async (req, res) => {
  try {
    ...
    if (String(req.user.sub) === String(userId)) {
      return res.status(400).json({ message: 'Owner cannot delete own account' })
    }
    ...
    await Promise.all([User.findByIdAndDelete(userId), Profile.deleteOne({ user: userId })])
